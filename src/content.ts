...
(function initYouTubeAspectRatio() {
  ...
  function findRightControls(): HTMLElement | null {
    ...
    if (player) {
      const rc = player.querySelector(".ytp-right-controls") as HTMLElement | null;
      if (rc) return rc;
    }

    // 2. Global right-controls
    const rc = document.querySelector(".ytp-right-controls") as HTMLElement | null;
    if (rc) return rc;
    ...
    return document.querySelector(".ytp-chrome-controls") as HTMLElement | null;
  }
